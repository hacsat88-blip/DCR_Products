...
| Cursor          | .cursor/rules/dcr-kernel.md + *.mdc | Generated from .ai/kernel/ + .ai/catalog/                        |
...
- Generated layer: deploy outputs and mirrors (`.cursor/rules/`, `.windsurf/`, `.claude/agents/`, `.codex/agents/`; large mirrors are Git-ignored)
...
- Do not move `.ai/catalog/rules/` or `.ai/catalog/skills/` into `.cursor/`
...
Shared runtime kernel source. Cursor and Windsurf runtime entrypoints are generated from this file.

.cursor/rules/dcr-kernel.md
Cursor generated entry point. DCR Kernel + triggers (a/i/r/s/d/p/q/sh/) + Permission model +
Safety boundaries + Pipeline gate chain + module behaviors inlined.
Source of truth is `.ai/kernel/dcr-kernel.md`; do not edit this file directly. This local mirror is Git-ignored.

.cursor/rules/*.mdc
Cursor agent rules + skills. deploy.ps1 が `.ai/catalog/rules/*.md` と `.ai/catalog/skills/*/SKILL.md` から自動生成。
rules は元ファイル名.mdc、skills は skill-{name}.mdc として出力。
生成物であり、source of truth ではない。手編集しない。Git 管理外。
...
- .ai/kernel/dcr-kernel.md + .ai/catalog/rules/*.md + .ai/catalog/skills/*/SKILL.md → ~/.cursor/rules/*.mdc + repo-local .cursor/rules/ (Cursor)
...
- do not treat `.cursor/rules/`, `.windsurf/`, `.claude/agents/`, or `.codex/agents/` as editable source paths

...
contract:
  preconditions:
    - "host environment can access a financial statements tool such as get_financials"
    - "target company or ticker is identified"
  postconditions:
    - "fair value assumptions and valuation output are summarized with caveats"
  invariants:
    - "state assumptions explicitly and separate model output from market price facts"
---

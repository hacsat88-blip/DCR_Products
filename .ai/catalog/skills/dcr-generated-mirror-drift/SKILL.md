...
description: "DCR repo の generated mirror / tracked entrypoint drift を診断する。`deploy.ps1`、`deploy.ps1 -Check`、GitHub Actions の generated-entrypoint failure、`.agents/skills`、`.windsurf/`、`.codex/agents`、`.claude/agents`、AGENTS/CLAUDE/Copilot instructions の同期ずれが出たときに使う。"
contract:
  preconditions:
...
この skill は修正そのものより先に、どこを直すべきかを確定するために使う。`.agents/skills` や `.windsurf/` などの生成先を見つけても、そこを正本として編集しない。
...
- `.windsurf/`, `.codex/agents`, `.claude/agents`, `C:\Users\hacsa\.agents\skills` の扱いが怪しい
...
2. まず narrow check を使う。
   ...
   - Windsurf: `.\deploy.ps1 -Check -Target windsurf`
   - 全体: `.\deploy.ps1 -Check`
...
3. 該当 path を分類する。
   ...
   - generated mirror: `.windsurf/`, `.codex/agents`, `.claude/agents`, `C:\Users\hacsa\.agents\skills`
...
| `Generated mirrors must stay untracked.` | `.windsurf/`, `.codex/agents`, or `.claude/agents` is tracked | `git ls-files .windsurf .codex/agents .claude/agents` | `.gitignore` and Git index |
| Windsurf quality failure | malformed template/generated output | `tools/adapters/windsurf.ps1` and `deploy.ps1 -Check -Target windsurf` | `templates/windsurf` or Windsurf adapter |
...
pwsh -ExecutionPolicy Bypass -File .\deploy.ps1 -Check -Target windsurf

...
description: "API・UI コンポーネント・モジュール境界の設計で、単一案に飛びつく前に 3〜5 個の根本的に異なる代替案をトレードオフ付きで生成する。並列サブエージェントが確実に使えるランタイムでは並列実行、それ以外（Codex / Claude Code / Cursor）では順次思考で同じ多視点比較を作る。"
contract:
  preconditions:
...
**並列サブエージェントが無い／確実でないランタイム** (Codex, Claude Code, Cursor - if parallel subagents are unavailable or uncertain, use sequential reasoning):

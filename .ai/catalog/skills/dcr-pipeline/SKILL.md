...
### 🔴 がある場合
```
🔴 重大な問題が [N]件 あります。修正後に再度 q/ を実行してください。
```

### 全パス時の誘導
```
✅ QA通過。全機能が正常に実装されています。
💡 sh/ でリリース判定に進めます
```
...
- **Signal protocol**: 各ゲートの判定結果を 🟢/🟡/🔴 で報告
- **Permission model**: 実装フェーズは 🟡 (execute → report)、ファイル作成は 🔴 (plan → approve)

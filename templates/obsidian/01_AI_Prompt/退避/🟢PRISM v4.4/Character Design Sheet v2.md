# Character Design Sheet v2

> キャラクター設計の外部ナレッジ。PRISM §3 段階的精緻化 > Character Blueprint から参照される。
> 対象モデルには依存しない。Structured Prompt段階でモデル別に変換する。

- Version: 2.0
- Date: 2026-02-28
- Changelog: v1→v2 — カラーパレット/服装パターン/アクセサリ詳細/ちびキャラ/Consistency Anchors追加。PRISM v4.4対応
...
- 本シートが未提供の場合、PRISMの最小定義（Character Core + Consistency Anchors）で進む
...
general_character_design_sheet:
  ...
  style:
    # 添付画像の作風を継承することを明示。特定のデフォルメやプロポーションは指定しない。
  ...
  color_palette:
    ...
    fields:
      ...
      - hair_color:
          ...
          value: "" # HEX必須。グラデーション時は "root:#xxx → tip:#xxx"
    constraints:
  ...
  costume_patterns:
    ...
    fields:
      ...
      - id: "costume_alt_1"
        label: "" # 例: "戦闘装備" / "私服" / "礼装"
        description: ""
        diff_from_default: "" # defaultとの差分のみ記述
      - id: "costume_alt_2"
        label: ""
        description: ""
        diff_from_default: ""
  ...
  accessories_and_equipment:
    ...
    fixed:
      ...
      items: [] # 例: [{name: "右耳のピアス", detail: "銀の三日月型"}, ...]
    variable:
      description: "シーンや衣装によって有無が変わる装備。"
      items: [] # 例: [{name: "腰の剣", detail: "戦闘シーンのみ", appears_in: ["costume_default", "costume_alt_1"]}]
    constraints:
      - "fixedアイテムは全パネル・全衣装で描写する。描き忘れは不整合として扱う。"
      - "variableアイテムはappears_inに記載された衣装/シーンでのみ描写する。"

  # ── v2 新設: ちびキャラ / SDバリエーション ──
  chibi_sd_variation:
    description: "デフォルメ版キャラクターの設計仕様。"
    fields:
      - head_body_ratio: "" # 例: "2頭身" / "2.5頭身" / "3頭身"
      - deformation_rules:
  ...
  # ── v2 新設: Consistency Anchors（PRISM §6 Mutation Thresholdと連動）──
  consistency_anchors:
    description: "キャラクターの同一性を維持するための固定点と可変点の定義。"
    immutable:
      description: "変更すると別キャラクターになる要素。全パネル・全衣装・全バリエーションで厳守。"
      items: [] # 3〜5要素。例: ["瞳の色と形", "髪のシルエット", "体格と頭身", "右耳のピアス", "声のトーン(設定上)"]
    mutable:
      description: "シーン・衣装・表情に応じて変更してよい要素。"
      items: [] # 3〜5要素。例: ["髪型のアレンジ", "服装", "表情", "ポーズ", "装備の有無(variable)"]
    carry_forward_tags:
      description: "連作時に次のシーンへ引き継ぐ状態タグ。"
      max: 3
      items: [] # 例: ["傷(右頬の切り傷・Episode 3以降)", "髪を切った(Episode 5以降)"]
    mutation_trigger: "immutableへの変更要求は、PRISMの再設計トリガーとして扱い、ユーザー承認を必須とする。"
    ...
    sections:
      ...
      - id: "expression_sheet"
        ...
        expressions:
          ...
          - expression: "" # カスタム枠1（キャラ固有表情。例: "冷笑", "照れ"）
            description: ""
          - expression: "" # カスタム枠2
            description: ""

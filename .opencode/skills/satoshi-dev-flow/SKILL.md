---
name: satoshi-dev-flow
description: "DCR Kernel のゲート連鎖 (p/ -> 実装 -> q/ -> sh/) をOpenCode環境で自動管理するパイプラインSkill。実装タスク開始時・完了時・リリース判定時に自動的に次ゲートへ誘導し、各ゲート通過条件をチェックする。Use when starting implementation tasks, completing features, or preparing for release."
---

# Satoshi Dev Flow Skill

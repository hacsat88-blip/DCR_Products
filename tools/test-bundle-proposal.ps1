...
<#
.SYNOPSIS
  Smoke test for Bundle Proposal V10.1.

.DESCRIPTION
  Verifies that V10 bundle advice can be converted into a short approval
  proposal and, when requested, persisted to gate-state proposal_state without
  applying any bundle change.
#>
...
$ProposalScript = Join-Path $RepoRoot "tools\bundle-proposal.ps1"
if (-not (Test-Path $ProposalScript)) {
    throw "bundle-proposal.ps1 not found: $ProposalScript"
}

$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("bundle-proposal-test-" + [System.Guid]::NewGuid().ToString("N"))
...
$PreviewJson = Join-Path $TempRoot "bundle-proposal-preview.json"
$CommitJson = Join-Path $TempRoot "bundle-proposal-commit.json"
$GateStatePath = Join-Path $TempRoot ".ai\routing\state\gate-state.json"
$CustomGateStatePath = Join-Path $TempRoot "custom\bundle-gate-state.json"
...
try {
    ...
    if (-not (Test-Path $PreviewJson)) {
        throw "preview bundle proposal JSON was not written"
    }
    ...
    if ($previewReport.options[0].kind -ne "bundle_policy") {
        throw "expected bundle_policy option kind"
    }
    ...
    Write-Host "bundle-proposal V10.1 smoke passed"
}
finally {

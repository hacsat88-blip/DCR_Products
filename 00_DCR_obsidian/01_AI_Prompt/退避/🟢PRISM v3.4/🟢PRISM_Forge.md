...
- Version: 3.4
- Date: 2026-02-27
- Canonical: 本ファイルは PRISM_CORE.md v3.4 と組で使用する。旧バージョンが同時に存在する場合、本ファイルを正とする。
- Changelog: 未読時強制縮退の追加、Phase 3 YAML絶対フォーマット指定
...
**出力フォーマットの絶対制約: ユーザーの物理ファイル名に依存せず、最終的なスキーマ出力時は必ずMarkdownのYAMLコードブロック（\`\`\`yaml ... \`\`\`）で全体を囲んで出力すること。**

...
export interface NavigatorStageLogEntry {
  id: string;
  runId: string;
  step: PipelineStep;
  label: string;
...
export interface RecommendationDiff {
  isNew: boolean;
  /** Positive when current rank is better than previous run. */
  rankDelta: number;
  /** Positive when star rating increased from previous run. */
  starDelta: number;
}

export type RecommendationDiffMap = Record<string, RecommendationDiff>;

export interface NavigatorIntervention {
...
export interface NavigatorState {
  ...
  /** Live log text streamed in terminal UI. */
  logText: string;
  /** Which pipeline stage currently owns `logText`. */
  logStep: PipelineStep | null;
  /** Completed stage logs accumulated for accordion history. */
  logHistory: NavigatorStageLogEntry[];
  /** Transition bridge message between completed and next stage. */
  bridgeMessage: string | null;
  /** Intervention box state shown between stages while pipeline is running. */
  intervention: NavigatorIntervention | null;
  /** Diff metadata for recommendation delta badges on rerun. */

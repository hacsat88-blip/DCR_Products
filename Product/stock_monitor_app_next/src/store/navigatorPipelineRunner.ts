...
import { DEFAULT_PIPELINE_STEPS } from "@/types/navigator";
...
class NavigatorStepError extends Error {
  readonly retry: NavigatorRetryState | null;

  constructor(message: string, retry: NavigatorRetryState | null = null) {
    super(message);
    this.name = "NavigatorStepError";
    this.retry = retry;
  }
}
...
function isRateLimitRetryState(retry: NavigatorRetryState | null): retry is NavigatorRetryState {
  return retry?.reason === "rate_limit";
}

function parsePositiveSeconds(value: number | null | undefined): number | null {
...
function parseRetryAt(value: string | null | undefined): string | null {
...
function resolveRateLimitRetry(
  response: Response,
  retry: NavigatorRetryState | null | undefined,
  nowMs: number
): NavigatorRetryState | null {
  const normalizedRetry = retry?.reason === "rate_limit"
    ? {
        reason: "rate_limit" as const,
        retryAfterSeconds: parsePositiveSeconds(retry.retryAfterSeconds),
        retryAt: parseRetryAt(retry.retryAt)
      }
    : null;
  const headerRetryAfterSeconds = parseRetryAfterHeader(response.headers.get("Retry-After"), nowMs);
  ...
  const retryAfterSeconds = normalizedRetry?.retryAfterSeconds ?? headerRetryAfterSeconds;
  const retryAt =
    normalizedRetry?.retryAt ??
    (retryAfterSeconds !== null ? new Date(nowMs + retryAfterSeconds * 1000).toISOString() : null);
...
export interface NavigatorPipelineStore extends NavigatorState {
  currentRunId: string | null;
  lastSuccessfulFinal: FinalEvaluation | null;
  closeModal: () => void;
}

type NavigatorPipelineSet<TStore extends NavigatorPipelineStore> = (
  partial:
    | Partial<TStore>
    | ((state: TStore) => Partial<NavigatorPipelineStore>),
) => void;

async function waitForInterventionGate<TStore extends NavigatorPipelineStore>(
  completedStep: PipelineStep,
  nextStep: PipelineStep,
  set: NavigatorPipelineSet<TStore>,
  get: () => TStore,
): Promise<string | null> {
  set({
    intervention: {
  ...
  while (true) {
    ...
    if (intervention.advanceRequested || intervention.remainingSeconds <= 0) {
      ...
      set({ intervention: null } as Partial<TStore>);
      return note;
    }
...
async function runStep<T extends StepResult>(
  ...
  if (!payload) {
    if (!response.ok) {
      const retry = resolveRateLimitRetry(response, null, nowMs);
      if (retry) {
        throw new NavigatorStepError(`HTTP ${response.status} (非JSON応答)`, retry);
      }
  ...
  if (!response.ok || payload.error || !payload.result) {
    ...
    throw new NavigatorStepError(truncate(reason), resolveRateLimitRetry(response, payload.retry ?? null, nowMs));
  }
...
interface RunNavigatorPipelineContext<TStore extends NavigatorPipelineStore> {
  set: NavigatorPipelineSet<TStore>;
  get: () => TStore;
  persist: () => void;
}

export async function runNavigatorPipeline<TStore extends NavigatorPipelineStore>({
  set,
  get,
  persist,
}: RunNavigatorPipelineContext<TStore>): Promise<void> {
  ...
  if (!settings || !settings.market || !settings.risk || !settings.horizon) {
    set({ status: "error", error: "すべての設定を入力してください" } as Partial<TStore>);
    return;
  }
  ...
  try {
    ...
    try {
      const step0 = await runStep<MacroResult>({ step: 0, settings, freeInput: settings.freeInput });
      if (step0.mock) {
      ...
      set((state) => ({
        ...
        status: "error",
        error: isRateLimitRetryState(retry)
          ? "AI分析は一時的に混雑しています。しばらく待って再実行してください。"
          : "マクロ分析に失敗しました (Step 0)。設定を確認して再実行してください。",
    ...
    try {
      const step1 = await runStep<StockSelectionResult>({ step: 1, settings, freeInput: settings.freeInput, macro });
      if (step1.mock) {
        throw new Error("mockフォールバック結果を検出したため停止しました");
      }
      stocks = step1.result;
    ...
    try {
      const step2 = await runStep<DebateResult>({ step: 2, settings, freeInput: settings.freeInput, macro, stocks });
      if (step2.mock) {
        throw new Error("mockフォールバック結果を検出したため停止しました");
      }
      debate = step2.result;
    ...
    try {
      const step3 = await runStep<FinalEvaluation>({
        step: 3,
        settings,
        freeInput: settings.freeInput,
        macro,
        stocks,
        debate,
      });

...
function StalenessBar({ lastUpdatedAt }: { lastUpdatedAt: string | null }): JSX.Element | null {
 ...
 const color = progress < 0.5 ?"bg-positive" : progress < 1 ?"bg-amber" :"bg-danger";

 return (
 <div className="h-[2px] w-16 rounded-lg bg-canvas-deep/80 overflow-hidden">
 <div
...
"inline-flex items-center gap-1.5 rounded-lg border px-3 py-1 font-medium",
 dataMode ==="live" &&"border-positive/25 bg-positive/8 text-positive",
...
"h-1.5 w-1.5 rounded-full",
 ...
 />
  ...
  <span className="font-mono tabular-nums text-[10px]">P:{sourceMeta.price}</span>
  <span className="font-mono tabular-nums text-[10px]">F:{sourceMeta.fundamentals}</span>
  <span className="font-mono tabular-nums text-[10px]">O:{sourceMeta.overall}</span>
  </span>
  ) : null}
 ...
 <span className="inline-flex items-center gap-2 rounded-lg border border-border-subtle bg-canvas-deep/50 px-3 py-1 text-text-muted">
 最終更新:
 <span className="font-mono tabular-nums text-text-secondary">{formatDateTime(lastUpdatedAt)}</span>
 <span className={clsx("font-medium font-mono tabular-nums", isStale ?"text-danger" :"text-text-secondary")}>
 ({elapsed})
 </span>
...
"inline-flex items-center gap-1.5 rounded-lg border px-3 py-1 font-medium",
 ...
 <span className={clsx("h-1.5 w-1.5 rounded-full", item.ok ?"bg-positive" :"bg-danger")} />
 {providerLabel(item.provider)}: {item.ok ?"正常" :"失敗"}
 {item.latencyMs != null && (
 <span className="font-mono tabular-nums text-text-muted">({item.latencyMs}ms)</span>
 )}
 </span>
 ))}
 ...
 <>
 <span className="hidden h-4 w-px bg-border-subtle sm:block" />
 <span className="inline-flex items-center gap-1.5 rounded-lg border border-secondary/20 bg-secondary/6 px-3 py-1 text-secondary">
 ⏱ 次回更新: {nextRefresh}
 </span>
 </>
 )}

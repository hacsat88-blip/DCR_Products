...
function sourceStaleThresholdMs(provider: ProviderHealth["provider"]): number {
 if (provider === "edinetDb") return 365 * 24 * 60 * 60 * 1000;
...
function ConnectionTest(): JSX.Element {
 ...
 disabled={testing}
 className="rounded-lg border border-border-subtle px-3 py-1 text-[11px] font-medium text-text-secondary transition-colors hover:border-border-active hover:text-text-primary disabled:opacity-50"
 >
 {testing ?"テスト中..." :"接続テスト"}
 </button>
 {results &&
...
"inline-flex items-center gap-1 text-[11px]",
 val.ok ?"text-positive" :"text-danger"
 )}
 >
 <span
 className={clsx(
"h-1.5 w-1.5 rounded-full",
 val.ok ?"bg-positive" :"bg-danger"
 )}
 />
...
function SummaryBarInner({
 ...
 <span
 className={clsx(
"inline-flex items-center gap-1.5 rounded-lg border px-3 py-1 text-[11px] font-medium",
 dataMode ==="live" &&"border-positive/30 bg-positive/8 text-positive",
...
"h-1.5 w-1.5 rounded-full",
 ...
 {isStale ? (
 <span className="inline-flex items-center gap-1.5 rounded-lg border border-amber/30 bg-amber/8 px-3 py-1 text-[11px] font-medium text-amber">
 <span className="h-1.5 w-1.5 rounded-full bg-amber" />
 取得時刻が古い
 </span>
 ) : null}
 {sourceStaleProviders.length > 0 ? (
 <span className="inline-flex items-center gap-1.5 rounded-lg border border-danger/30 bg-danger/8 px-3 py-1 text-[11px] font-medium text-danger">
 ...
 <p className="mt-2 text-xs text-text-muted">最終更新: <span className="font-mono tabular-nums text-text-secondary">{formatDateTime(lastUpdatedAt)}</span></p>
  {providerStatusSummary ? (
  <p className="mt-1 text-xs text-text-muted">取得状況: <span className="text-text-secondary">{providerStatusSummary}</span></p>
  ) : null}
  {sourceMeta ? (
  <p className="mt-1 text-xs text-text-muted">ソース: <span className="font-mono text-text-secondary">P:{sourceMeta.price} / F:{sourceMeta.fundamentals} / O:{sourceMeta.overall}</span></p>
  ) : null}
...
"rounded-lg border px-3 py-2.5 text-xs",
 ...
 <p className="mt-3 text-xs text-text-muted">価格: Yahoo Finance（fallback: Alpha Vantage）/ 財務: EDINET DB</p>
 <ConnectionTest />
 </section>
 );
}

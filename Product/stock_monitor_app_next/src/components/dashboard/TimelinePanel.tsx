...
function TimelinePanelInner({ stock, snapshots }: TimelinePanelProps): JSX.Element {
 ...
 <linearGradient id="gradientScore" x1="0" y1="0" x2="0" y2="1">
  <stop offset="0%" stopColor={CHART_SERIES.score} stopOpacity={0.2} />
  <stop offset="100%" stopColor={CHART_SERIES.score} stopOpacity={0} />
  </linearGradient>
  </defs>
  ...
  <YAxis yAxisId="score" stroke="transparent" tick={{ ...CHART_AXIS_TICK, fill: CHART_SERIES.score }} tickLine={false} axisLine={false} width={36} domain={[0, 100]} />
  <YAxis yAxisId="price" orientation="right" stroke="transparent" tick={{ ...CHART_AXIS_TICK, fill: CHART_SERIES.price }} tickLine={false} axisLine={false} width={50} tickFormatter={(v: number) => `¥${(v / 1000).toFixed(0)}k`} />
 ...
 x={change.checkedAt}
 yAxisId="score"
  stroke={CHART_SERIES.signal}
  strokeDasharray="4 4"
  strokeWidth={1}
  label={{ value: actionLabel(change.action), position:"insideTopRight", fontSize: 9, fill: CHART_SERIES.signal }}
  />
  ))}
  <Area yAxisId="score" type="monotone" dataKey="score" fill="url(#gradientScore)" stroke="none" />
  <Line yAxisId="score" type="monotone" dataKey="score" name="本命度" stroke={CHART_SERIES.score} strokeWidth={2} dot={false} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.score }} />
  <Line yAxisId="price" type="monotone" dataKey="price" name="株価" stroke={CHART_SERIES.price} strokeWidth={2} dot={{ r: 2, fill: CHART_SERIES.price, strokeWidth: 0 }} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.price }} />
 </LineChart>
 </ResponsiveContainer>
 </div>

...
function BenchmarkLegend(): JSX.Element {
  const items = [
  { label:"戦略指数", color: CHART_SERIES.strategy },
  { label:"ベンチマーク指数", color: CHART_SERIES.benchmark },
  ];
 return (
...
export const BenchmarkChart = memo(function BenchmarkChart({ stocks }: BenchmarkChartProps): JSX.Element {
 ...
 return (
 <section className="rounded-lg border border-glass-border bg-panel p-5 shadow-card card-surface-hover">
 <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
 <h2 className="text-lg font-semibold text-text-primary">ベンチマーク比較チャート</h2>
 <p className="rounded-lg border border-border-subtle/40 px-3 py-1 text-xs text-text-muted">指数表示（初期日=100）</p>
 </div>
 ...
 <linearGradient id="gradientManager" x1="0" y1="0" x2="0" y2="1">
  ...
  <stop offset="0%" stopColor={CHART_SERIES.benchmark} stopOpacity={0.15} />
  <stop offset="100%" stopColor={CHART_SERIES.benchmark} stopOpacity={0} />
  </linearGradient>
 </defs>
 ...
 <Area type="monotone" dataKey="benchmark" fill="url(#gradientBenchmark)" stroke="none" />
  <Line type="monotone" dataKey="manager" name="戦略指数" stroke={CHART_SERIES.strategy} strokeWidth={2} dot={false} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.strategy }} animationDuration={800} animationEasing="ease-out" />
  <Line type="monotone" dataKey="benchmark" name="ベンチマーク指数" stroke={CHART_SERIES.benchmark} strokeWidth={2} dot={false} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.benchmark }} animationDuration={800} animationEasing="ease-out" />
 </LineChart>
 </ResponsiveContainer>
 </div>
 <BenchmarkLegend />

...
const LIVE_TIMEFRAMES = new Set<CandleTimeframe>(["5m","15m","1h"]);
...
function CandlestickChartInner(
 ...
 const sourceLabel = SOURCE_LABELS[source] ?? source;
  ...
  LIVE_TIMEFRAMES.has(timeframe) && source ==="alpha_vantage" && error === null && ohlc.length > 0;
 ...
 {sourceLabel && (
  ...
  {liveDataActive && (
  <span className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-positive/30 bg-positive/10 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider text-positive">
...
"cyber-btn whitespace-nowrap px-2.5 py-1 text-[10px]",
 ...
 <p className="font-mono tabular-nums text-sm text-danger">
 ⚠ エラーが発生しました
 </p>
 ...
 <div className="flex items-center gap-2">
 <svg
 className="h-4 w-4 animate-spin-slow text-positive/50"
 viewBox="0 0 24 24"
 fill="none"
 >
 <circle
 ...
 <p className="font-mono tabular-nums text-sm text-text-muted">
 データなし
 </p>
 </div>
 ) : null}

...
function MorningCheckPanelInner({ stocks, snapshots }: MorningCheckPanelProps): JSX.Element {
 ...
 <div>
 <h2 className="text-lg font-semibold text-text-primary">朝チェックモード</h2>
 ...
 {visibleRows.map((row) => (
 <article key={row.code} className="rounded-lg border border-border-subtle bg-canvas-deep/60 p-3">
 ...
 className={clsx(
"mt-1 text-xs",
 row.scoreDelta === null ?"text-text-muted" : row.scoreDelta > 0 ?"text-positive" :"text-danger"
 )}
 >
 スコア差分:{""}
 <span className="font-mono tabular-nums">{row.scoreDelta === null ?"-" : `${row.scoreDelta > 0 ?"+" :""}${row.scoreDelta.toFixed(1)}`}</span>
 </p>
 <p className="mt-1 text-xs text-text-secondary">
 価格差分:{""}
 <span className="font-mono tabular-nums">{row.priceDeltaPct === null
 ?"-"

...
import { ACTION_CHART_COLORS, CHART_AXIS_TICK, CHART_COLORS, CHART_SECTOR_COLORS, CHART_SERIES, CHART_TOOLTIP_STYLE } from"@/components/ui/ChartTheme";
import { Badge } from"@/components/ui/Badge";
import { SrcMetaDots } from "@/components/ui/SrcDot";

/* ───────── palette ───────── */
const SECTOR_COLORS = [...CHART_SECTOR_COLORS];

const ACTION_COLOR: Record<string, string> = {
 buy_now: ACTION_CHART_COLORS.buy_now,
 wait_earnings: ACTION_CHART_COLORS.wait_earnings,
 wait_pullback: ACTION_CHART_COLORS.wait_pullback,
...
type SortField ="value" |"score" |"shares" |"weight" |"dailyPnL";

/* ───────── tooltip style ───────── */
const tooltipStyle: React.CSSProperties = {
  ...CHART_TOOLTIP_STYLE,
  backdropFilter:"blur(8px)",
  color: CHART_AXIS_TICK.fill,
};
...
function previousClosePrice(price: number | null, changePercent: number | null): number | null {
...
function BarTooltipContent({ active, payload }: { active?: boolean; payload?: Array<{ payload: { name: string; value: number; changePercent: number | null } }> }) {
 if (!active || !payload?.[0]) return null;
  const d = payload[0].payload;
  const color = d.changePercent === null ? CHART_COLORS.axis : d.changePercent >= 0 ? CHART_SERIES.positive : CHART_SERIES.negative;
 return (
...
function PortfolioPanelInner(): JSX.Element {
 ...
 /* ── derived data ── */
  ...
  const previousClose = previousClosePrice(price, changePercent);
  const dailyPnL = previousClose === null || price === null ? null : shares * (price - previousClose);
  rows.push({ stock, shares, price, value, previousClose, dailyPnL, changePercent, weight: 0 });
  }
  const total = rows.reduce((s, r) => s + (r.value ?? 0), 0);
  for (const r of rows) {
  r.weight = total > 0 && r.value !== null ? r.value / total : 0;
  }
  return rows;
  }, [stocks, holdingsMap]);

  const totalValue = useMemo(() => holdingRows.reduce((s, r) => s + (r.value ?? 0), 0), [holdingRows]);
  ...
  const dailyChange = useMemo(() => holdingRows.reduce((s, r) => s + (r.dailyPnL ?? 0), 0), [holdingRows]);

  const previousTotal = useMemo(
    () => holdingRows.reduce((s, r) => s + (r.previousClose === null ? 0 : r.previousClose * r.shares), 0),
    [holdingRows]
  );

  const dailyChangePct = useMemo(
  () => (previousTotal > 0 ? (dailyChange / previousTotal) * 100 : null),
  [dailyChange, previousTotal],
  );
  const hasDailyChange = previousTotal > 0;

  /* sector pie data */
  const sectorData = useMemo<SectorSlice[]>(() => {
  const map = new Map<string, number>();
  for (const r of holdingRows) {
  map.set(r.stock.sector, (map.get(r.stock.sector) ?? 0) + (r.value ?? 0));
  }
 return [...map.entries()]
 .sort((a, b) => b[1] - a[1])
 .map(([name, value], i) => ({
 ...
 /* bar chart data */
  ...
  const sortable = (value: number | null): number => (value === null ? Number.NEGATIVE_INFINITY : value);
  rows.sort((a, b) => {
  let diff = 0;
  switch (tableSortField) {
  case"value":
  diff = sortable(a.value) - sortable(b.value);
  break;
  case"dailyPnL":
  diff = sortable(a.dailyPnL) - sortable(b.dailyPnL);
  break;
  case"score":
 ...
 return <span className="ml-1 text-positive">{tableSortAsc ?"↑" :"↓"}</span>;
 }
 ...
 <div className="absolute inset-y-0 left-0 w-[3px] bg-positive/50" />
 <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-text-muted">
 総評価額
 </p>
 <p className="mt-2 text-3xl font-semibold font-mono tabular-nums tracking-kpi text-text-primary md:text-4xl">
 ...
 {/* 銘柄数 */}
 <div className="card-surface relative overflow-hidden p-5">
 <div className="absolute inset-y-0 left-0 w-[3px] bg-secondary/50" />
 <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-text-muted">
 銘柄数
 </p>
 <p className="mt-2 text-3xl font-semibold font-mono tabular-nums tracking-kpi text-text-primary md:text-4xl">
 ...
 <p
 className={clsx(
"mt-2 text-3xl font-semibold font-mono tabular-nums tracking-kpi md:text-4xl",
 ...
 "absolute inset-y-0 left-0 w-[3px]",
  !hasDailyChange ?"bg-border-subtle/60" : dailyChange >= 0 ?"bg-positive/50" :"bg-danger/50",
  )}
  />
 <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-text-muted">
 前日比
  ...
  className={clsx(
 "mt-2 text-3xl font-semibold font-mono tabular-nums tracking-kpi md:text-4xl",
  !hasDailyChange ?"text-text-muted" : dailyChange >= 0 ?"text-positive" :"text-danger",
  )}
  >
  {hasDailyChange ? signedYen(dailyChange) : "-"}
  </p>
  <p
  className={clsx(
 "mt-2 text-xs font-mono tabular-nums",
  !hasDailyChange ?"text-text-muted" : dailyChange >= 0 ?"text-positive/70" :"text-danger/70",
  )}
  >
 ...
 <Cell
 key={entry.name}
  fill={ACTION_COLOR[entry.action] ?? CHART_COLORS.violet}
  />
 ))}
 </Bar>
 </BarChart>
 ...
 <td className="py-2.5 pr-3 text-text-secondary font-mono tabular-nums">
 {r.shares.toLocaleString("ja-JP")}
 </td>
   <td className="py-2.5 pr-3 text-text-secondary font-mono tabular-nums">
   {r.price === null ? "-" : formatYen(r.price)}
   </td>
   <td className="py-2.5 pr-3">
   <SrcMetaDots priceLabel={r.stock.priceSourceLabel} fundamentalsLabel={r.stock.fundamentalsSourceLabel} />
   </td>
   <td className="py-2.5 pr-3 font-medium font-mono tabular-nums text-text-primary">
   {r.value === null ? "-" : formatYen(Math.round(r.value))}
   </td>
  <td
  className={clsx(
  "py-2.5 pr-3 font-medium font-mono tabular-nums",
  r.dailyPnL === null ?"text-text-muted" : r.dailyPnL >= 0 ?"text-positive" :"text-danger",
  )}
  >
  <div className="flex flex-col gap-0.5">
  <span>{signedYen(r.dailyPnL)}</span>
  ...
  <td className="py-2.5 pr-3 text-text-secondary font-mono tabular-nums">
  {r.value === null ? "-" :`${(r.weight * 100).toFixed(1)}%`}
  </td>
 <td className={clsx("py-2.5 pr-3 font-semibold font-mono tabular-nums", scoreColor(r.stock.score))}>
 ...
 <span className="text-positive">{actionCounts.buy_now}銘柄 今買う</span>
 )}
 {actionCounts.wait_earnings > 0 && (
 <span className="text-secondary">{actionCounts.wait_earnings}銘柄 決算待ち</span>
 )}
 {actionCounts.wait_pullback > 0 && (
 <span className="text-secondary">{actionCounts.wait_pullback}銘柄 押し目待ち</span>
 )}
 ...
 <p className={clsx("text-2xl font-semibold font-mono tabular-nums", scoreColor(weightedAvgScore))}>
 {weightedAvgScore.toFixed(1)}
 </p>
 </div>
 ...
 <span className={clsx("text-sm font-semibold font-mono tabular-nums", scoreColor(lowestScoreRow.stock.score))}>

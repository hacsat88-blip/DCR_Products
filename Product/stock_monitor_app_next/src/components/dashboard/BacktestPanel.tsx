...
function BacktestLegend(): JSX.Element {
  const items = [
  { label:"戦略指数", color: CHART_SERIES.strategy, dashed: false },
  { label:"ベンチマーク", color: CHART_SERIES.benchmark, dashed: true },
  ];
 return (
...
function BacktestPanelInner({
 ...
 setStockCode(stocks[0]?.code ??"");
  ...
  () => stocks.find((stock) => stock.code === stockCode) ?? null,
  [stocks, stockCode]
  );
 ...
 <div>
 <h2 className="text-lg font-semibold text-text-primary">バックテスト</h2>
 ...
 結果クリア
  ...
  className="rounded-lg border border-border-subtle bg-canvas-deep/70 px-2 py-2 text-xs text-text-primary"
 >
 {stocks.map((stock) => (
 <option key={stock.code} value={stock.code}>
  ...
  {selectedStock ? (
  <SrcMetaDots
  priceLabel={selectedStock.priceSourceLabel}
  fundamentalsLabel={selectedStock.fundamentalsSourceLabel}
  />
  ) : null}
  </div>
 ...
 className="rounded-lg border border-secondary/40 bg-secondary/10 px-3 py-2 text-xs font-semibold text-secondary disabled:cursor-not-allowed disabled:opacity-50"
 >
 バックテスト実行
 </button>
 </div>
 ...
 <div className="rounded-lg border border-border-subtle bg-canvas-deep/50 p-3">
 <p className="text-[10px] font-medium uppercase tracking-widest text-text-muted">戦略リターン</p>
 <p className={clsx("mt-1 text-base font-bold tabular-nums font-mono", (current?.totalReturnPct ?? 0) >= 0 ?"text-positive" :"text-danger")}>
 ...
 <p className={clsx("mt-1 text-base font-bold tabular-nums font-mono", (current?.benchmarkReturnPct ?? 0) >= 0 ?"text-positive" :"text-danger")}>
 ...
 <p className={clsx("mt-1 text-base font-bold tabular-nums font-mono", (current?.excessReturnPct ?? 0) >= 0 ?"text-positive" :"text-danger")}>
 ...
 <p className="mt-1 text-base font-bold tabular-nums text-danger font-mono">
 ...
 <p className="mt-1 text-base font-bold tabular-nums text-text-primary font-mono">{current?.actionChanges ?? 0}</p>
 </div>
 </div>
 ...
 <linearGradient id="gradientStrategy" x1="0" y1="0" x2="0" y2="1">
  <stop offset="0%" stopColor={CHART_SERIES.strategy} stopOpacity={0.2} />
  <stop offset="100%" stopColor={CHART_SERIES.strategy} stopOpacity={0} />
  </linearGradient>
 </defs>
 ...
 <Area type="monotone" dataKey="strategy" fill="url(#gradientStrategy)" stroke="none" />
  <Line type="monotone" dataKey="strategy" name="戦略指数" stroke={CHART_SERIES.strategy} strokeWidth={2} dot={false} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.strategy }} />
  <Line type="monotone" dataKey="benchmark" name="ベンチマーク" stroke={CHART_SERIES.benchmark} strokeWidth={2} strokeDasharray="6 3" dot={false} activeDot={{ ...ACTIVE_DOT_PROPS, stroke: CHART_SERIES.benchmark }} />
 </LineChart>
 </ResponsiveContainer>
 </div>
 <BacktestLegend />

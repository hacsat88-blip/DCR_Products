...
import { CHART_COLORS, CHART_AXIS_TICK, CHART_TOOLTIP_STYLE, CHART_SERIES } from "@/components/ui/ChartTheme";
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            ...
            <h3 id={`stock-detail-title-${stock.id}`} className="text-2xl font-bold tracking-heading text-text-primary">{displayName}</h3>
        ...
        <CollapsibleSection title="会社概要" defaultOpen>
          <div className="card-surface p-4">
            <p className="text-sm leading-7 text-text-primary">{insightText}</p>
            ...
            <p className="mt-1 text-sm leading-7 text-text-primary">{overviewText}</p>
          ...
          <div className="h-[320px]">
            <ResponsiveContainer>
              <LineChart data={stock.chartData} margin={{ top: 8, right: 10, left: 0, bottom: 8 }}>
                ...
                <YAxis stroke="transparent" tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} width={36} />
                <Tooltip
                  contentStyle={{
                    ...CHART_TOOLTIP_STYLE,
                ...
                />
                <Line type="monotone" dataKey="price" name="銘柄" stroke={CHART_SERIES.strategy} strokeWidth={2.5} dot={false} />
                <Line type="monotone" dataKey="benchmark" name="ベンチマーク" stroke={CHART_SERIES.benchmark} strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CollapsibleSection>
        ...
        <CollapsibleSection title="保有情報" defaultOpen>
          <HoldingSection stockId={stock.id} price={stock.price} changePercent={stock.changePercent} />
        </CollapsibleSection>
        ...
        <CollapsibleSection title="主要指標">
          <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <dd className="font-mono tabular-nums text-text-primary font-medium">{formatStockPriceDisplay(stock)}</dd>
            </div>
            <div>
              <dt className="text-text-muted text-xs">前日比</dt>
              <dd className="font-mono tabular-nums text-text-primary font-medium">{formatStockChangeDisplay(stock)}</dd>
            </div>
            <div>
...
function HoldingSection({ stockId, price, changePercent }: { stockId: string; price: number; changePercent: number | null }): JSX.Element {
  ...
  return (
    <div className="card-surface p-4">
      <HoldingInput stockId={stockId} price={price} changePercent={changePercent} />

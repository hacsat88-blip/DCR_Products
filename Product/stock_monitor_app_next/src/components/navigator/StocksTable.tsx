...
type SortKey = "code" | "price" | "signal" | "fcfYield" | "confidence";
...
export function StocksTable({ stocks, debate }: StocksTableProps): JSX.Element {
  ...
  const verdictMap = useMemo(() => {
    const map = new Map<string, DebateVerdict>();
  ...
  const sortedRows = useMemo(() => {
    ...
    rows.sort((a: SelectedStock, b: SelectedStock) => {
      switch (sortKey) {
        ...
        case "signal": {
          const sa = verdictMap.get(a.code)?.signal ?? "watch";
          const sb = verdictMap.get(b.code)?.signal ?? "watch";
          return (SIGNAL_RANK[sa] - SIGNAL_RANK[sb]) * dir;
        }
        case "confidence": {
          const va = verdictMap.get(a.code);
          const vb = verdictMap.get(b.code);
          const ca = va ? resolveDebateConfidence(va) : 0;
          const cb = vb ? resolveDebateConfidence(vb) : 0;
          return (ca - cb) * dir;
        }
        default:
          return 0;
    ...
    return rows;
  }, [stocks, sortKey, sortDir, verdictMap]);
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-glass-border">
              {[
                ...
                { key: "confidence" as SortKey, label: "CONF", w: "w-36" },
          ...
          <tbody>
            {sortedRows.map((stock, idx) => {
              const verdict = verdictMap.get(stock.code);
              const signal = verdict?.signal ?? "watch";
              const cfg = SIGNAL_STYLE[signal];
              const confidence = verdict ? resolveDebateConfidence(verdict) : 0;

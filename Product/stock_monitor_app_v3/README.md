...
JQUANTS_API_KEY=your_jquants_v2_api_key_here
...
JQUANTS_PRICE_CACHE_TTL_SECONDS=1800
...
- J-Quants 価格認証は `JQUANTS_API_KEY`（V2 `x-api-key`）が正規入口
- EDINET APIキー未設定時は `mock/fallback` で継続動作
- `JQUANTS_REFRESH_TOKEN` / `JQUANTS_ID_TOKEN` は価格認証では使用しない（旧運用）
...
## J-Quants V2 トラブルシュート

- `J-Quants API key missing` が出るとき:
  - `.env.local` に `JQUANTS_API_KEY=...` があるか確認
  - 変数名の誤り（`JQUANTS_REFRESH_TOKEN` / `JQUANTS_ID_TOKEN` のみ設定）を確認
- `J-Quants unauthorized` が出るとき:
  - 値が J-Quants V2 の API key か確認
  - refresh token / 旧 token を誤って設定していないか確認

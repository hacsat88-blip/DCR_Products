...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    ...
    const res = await fetch(
      "https://api.jquants.com/v2/listed/info?code=9424",
      { headers: { "x-api-key": apiKey }, signal: AbortSignal.timeout(5000) }
    );
    results.jquants = { ok: res.ok, latencyMs: Date.now() - jqStart };
    if (!res.ok) results.jquants.error = `HTTP ${res.status}`;
  } catch (e: unknown) {
    results.jquants = {
      ok: false,
      latencyMs: Date.now() - jqStart,

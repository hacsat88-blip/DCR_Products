...
function ExcessBars({
 ...
 <Cell
 key={i}
 fill={entry.d >= 0 ?"rgba(34,197,94,0.6)" :"rgba(239,68,68,0.6)"}
 />
 ))}
 </Bar>
 </BarChart>
...
function KpiCardsInner({
 ...
 <Sparkline data={managerSparkData} color="#22C55E" gradientId="spark-manager" />
 </div>
 ...
 <p className="mt-1 text-xs text-text-muted">初期日=100</p>
 <Sparkline data={benchmarkSparkData} color="#06B6D4" gradientId="spark-bench" />
 </div>
...
"mt-1 text-xs",
 ...
 <Sparkline data={nikkeiSparkData} color="#F59E0B" gradientId="spark-nikkei" />
 )}
 </div>
 </section>
 );

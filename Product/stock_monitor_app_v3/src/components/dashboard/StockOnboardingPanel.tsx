...
export function StockOnboardingPanel(): JSX.Element {
 ...
 <div className="mb-3">
 <h2 className="text-lg font-semibold text-text-primary">銘柄追加オンボーディング</h2>
 ...
 className="h-52 w-full rounded-lg border border-border-subtle bg-canvas/90 px-3 py-3 font-mono text-xs text-text-primary outline-none"
 />

 <div className="mt-3 rounded-lg border border-border-subtle bg-canvas-deep/60 p-3 text-xs">
 {validation.error ? <p className="text-danger">チェック結果: {validation.error}</p> : null}
 {!validation.error && validation.valid ? (
 <p className="text-positive">チェック結果: OK（必須項目が揃っています）</p>
 ) : null}
 {!validation.error && !validation.valid ? (

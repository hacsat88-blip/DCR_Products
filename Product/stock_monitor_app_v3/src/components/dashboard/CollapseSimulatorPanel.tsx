...
function ImpactZone({ current, simulated }: { current: number; simulated: number | null }): JSX.Element {
 ...
 <div className="relative mt-1 h-2 overflow-hidden rounded-lg">
 <div className="absolute inset-0 flex">
 <div className="h-full flex-[40] bg-danger/20" />
 <div className="h-full flex-[30] bg-amber/20" />
 <div className="h-full flex-[30] bg-positive/20" />
 </div>
 <div
 className="absolute top-0 h-full w-1 rounded-lg bg-text-primary"
 style={{ left: `${clampPos(current)}%` }}
 title={`現在: ${current}`}
 />
 {simulated !== null && (
 <div
 className="absolute top-0 h-full w-1 rounded-lg bg-amber opacity-80"
...
export function CollapseSimulatorPanel({
 ...
 <div className="mb-3">
 <h2 className="text-lg font-semibold text-text-primary">崩れシミュレーター</h2>
 ...
 <label className="rounded-lg border border-border-subtle bg-canvas-deep/50 p-3 text-xs text-text-primary">
 <span className="flex items-center justify-between">
 <span>売上成長補正</span>
 <span className="font-mono font-semibold text-positive">{revenueDelta > 0 ?"+" :""}{revenueDelta}</span>
 </span>
 <input
 type="range"
 min={-30}
 ...
 <span className="flex items-center justify-between">
 <span>営業利益成長補正</span>
 <span className="font-mono font-semibold text-secondary">{opDelta > 0 ?"+" :""}{opDelta}</span>
 </span>
 <input
 type="range"
 min={-30}
 ...
 <div className="rounded-lg border border-border-subtle bg-canvas-deep/50 p-3 text-xs text-text-primary">
 <p className="text-text-muted">現在</p>
 <p className={clsx("mt-1 text-lg font-bold tabular-nums font-mono", zoneColor(stock.score))}>{stock.score}</p>
 <p className="mt-1 text-text-secondary">{formatActionLabel(stock.evaluatedAction)}</p>
 </div>
 <div className={clsx(
"rounded-lg border p-3 text-xs text-text-primary transition-all duration-300",
 bigChange ?"border-amber/40 bg-canvas-deep/50" :"border-border-subtle bg-canvas-deep/50"
 )}>
 <p className="text-text-muted">シミュレーション後</p>
 <p className={clsx("mt-1 text-lg font-bold tabular-nums font-mono", simulated ? zoneColor(simulated.score) :"text-text-muted")}>
 {simulated?.score ??"-"}
 </p>
...
"mt-1 font-mono text-[11px] font-semibold",
 scoreDiff > 0 ?"text-positive" : scoreDiff < 0 ?"text-danger" :"text-text-muted"
 )}>

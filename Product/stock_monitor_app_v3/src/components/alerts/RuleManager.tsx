  ...
  return (
    <section className="rounded-none border border-border-subtle bg-panel p-5 shadow-card">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold font-orb text-text-primary">アラートルール管理</h2>
        <button
          type="button"
      ...
      <div className="mb-4 grid gap-2 md:grid-cols-3">
        {Object.values(ALERT_PRESET_CATALOG).map((preset) => (
          <button
            ...
            className="rounded-none border border-border-subtle bg-canvas-deep/60 px-3 py-3 text-left"
          >
      ...
      <div className="rounded-none border border-border-subtle bg-canvas-deep/60 p-3">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-4 xl:grid-cols-7">
          <select
            ...
            className="rounded-none border border-border-subtle bg-canvas/90 px-2 py-2 text-xs text-text-primary"
          >
          ...
          <select
            ...
            className="rounded-none border border-border-subtle bg-canvas/90 px-2 py-2 text-xs text-text-primary disabled:opacity-50"
          >
            {stocks.map((stock) => (
        ...
        <button
          type="button"
          disabled={!canAddRule}
          className="mt-3 rounded-none border border-blue/40 bg-blue/10 px-3 py-2 text-xs font-semibold text-blue disabled:cursor-not-allowed disabled:opacity-50"
          onClick={() =>
            onAddRule({
              type,
              scope,
      ...
      <div className="mt-4 grid gap-2">
        {sortedRules.map((rule) => (
          <article
            key={rule.id}
            className="rounded-none border border-border-subtle bg-canvas-deep/50 p-3"
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                ...
                className="rounded-none border border-border-subtle px-2 py-1 text-xs text-slate-200"
              >
                削除
              </button>
            </div>
            ...
            <div className="mt-2 grid gap-2 text-xs md:grid-cols-7">
              ...
              <select
                ...
                className="rounded-none border border-border-subtle bg-canvas/90 px-2 py-1 text-xs text-text-primary"
              >
                <option value="global">全体</option>
                <option value="stock">銘柄単位</option>
                <option value="watchlist">ウォッチリスト</option>
              ...
              <select
                ...
                className="rounded-none border border-border-subtle bg-canvas/90 px-2 py-1 text-xs text-text-primary disabled:opacity-50"
              >
                <option value="">-</option>
                {stocks.map((stock) => (
                  <option key={stock.code} value={stock.code}>

...
function severityTone(severity: AlertEvent["severity"]): string {
  ...
  return "border-blue/50 bg-blue/10 text-blue";
}
...
function priorityTone(priority: "high" | "medium" | "low"): string {
  ...
  if (priority === "low") return "border-slate-500 bg-slate-800/70 text-slate-200";
  return "border-blue/50 bg-blue/10 text-blue";
}
  ...
  return (
    <section className="rounded-2xl border border-slate-700/60 bg-panel p-5 shadow-card">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">アラートセンター</h2>
          <p className="text-xs text-slate-400">未読 {unreadCount} / 最終評価: {formatDateTime(lastEvaluationAt)}</p>
        </div>
        <button
          type="button"
          onClick={onClear}
      ...
      {sorted.length === 0 ? (
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 px-4 py-5 text-sm text-slate-300">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className={clsx("rounded-full border px-2 py-1 text-[11px] font-semibold", severityTone(event.severity))}>
                ...
                <p className="text-xs text-slate-400">{formatDateTime(event.triggeredAt)}</p>
              </div>
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                {!event.dismissed ? (
                  ...
                  <span className="text-xs text-slate-500">非表示済み</span>
                )}
              </div>
            </article>
          ))}

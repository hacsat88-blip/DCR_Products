  ...
  useEffect(() => {
    ...
    wsRef.current.onmessage = (e) => {
      ...
      if (data.daily_pnl !== undefined) {
        setStatus((prev) => prev ? { ...prev, daily_pnl: data.daily_pnl, risk_budget: data.risk_budget } : prev);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 font-mono">
      ...
      <div className="flex items-center justify-between mb-4 bg-gray-900 rounded-xl p-4">
        ...
        <div className="flex gap-2">
          ...
          {status?.trading_stopped && (
            <span className="bg-red-700 text-red-200 px-3 py-1 rounded-full text-xs">
      ...
      <div className="bg-gray-900 rounded-xl p-4 mb-4">
        ...
        <div className="text-right text-xs text-gray-500 mt-1">{riskUsed.toFixed(0)}% / 上限 ¥3,000</div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="px-4 py-2 bg-red-800 hover:bg-red-700 rounded-lg text-sm font-bold text-white transition-colors"
        >
          緊急停止
        </button>
      </div>

...
function New-ProjectInstructionOutput {
    ...
    $parts = @()
    if ($TargetName -eq "windsurf") {
        $parts += "---"
        $parts += "trigger: always_on"
        $parts += "description: DCR kernel baseline for Windsurf Cascade"
        $parts += "---"
        $parts += ""
    }
    ...
    $parts += ""
    $parts += "> Generated by init-project.ps1 from .ai/kernel/dcr-kernel.md, .ai/kernel/environments/, and templates/."
    $parts += ""

...
.DESCRIPTION
  ...
  共有リソース（-SkipShared で省略可）:
    .ai/core/*                      (DCR Core Kernel)
    .ai/routing/*                   (ルーティングモジュール)
...
.PARAMETER SkipShared
  共有リソース（.ai/core/, .ai/routing/）のコピーをスキップ
...
$Templates = @{
    claude   = @{
        Source = Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\CLAUDE.md"
        Dest   = Join-Path $ProjectPath ".claude\CLAUDE.md"
    }
    codex    = @{
        Source = Join-Path $RepoRoot ".ai\adapters\codex\templates\AGENTS.md"
        Dest   = Join-Path $ProjectPath "AGENTS.md"
    }
    copilot  = @{
        Source = Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.github\copilot-instructions.md"
...
function New-ProjectInstructionOutput {
    ...
    if ($environmentFile) {
        # Map adapter name to its kernel.md in .ai/adapters/<env>/kernel.md
        $adapterNameMap = @{
            "claude-code.md"      = "claude-code"
            "codex.md"            = "codex"
            "vscode-copilot.md"   = "vscode-copilot"
        }
        $adapterName = $adapterNameMap[$environmentFile]
        if ($adapterName) {
            $environmentPath = Join-Path $RepoRoot ".ai\adapters\$adapterName\kernel.md"
    ...
    $parts += ""
    $parts += "> Generated by init-project.ps1 from .ai/core/kernel.md, .ai/adapters/, and .ai/adapters/<env>/templates/."
    $parts += ""
...
foreach ($t in $targets) {
    ...
    if ($DryRun) {
        ...
        if ($t -eq "claude") {
            $claudeSettingsSource = Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\settings.local.json"
            ...
            $claudeExtraFiles = @(
                @{ Source = Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\mcp_config.example.json"; Dest = Join-Path $ProjectPath ".claude\mcp_config.example.json" },
                @{ Source = Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\hooks.example.json"; Dest = Join-Path $ProjectPath ".claude\hooks.example.json" }
            )
            ...
            $claudeCommandsSourceDir = Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\commands"
        ...
        if ($t -eq "codex") {
            $codexExtraFiles = @(
                @{ Source = Join-Path $RepoRoot ".ai\adapters\codex\templates\.codex\mcp_config.example.json"; Dest = Join-Path $ProjectPath ".codex\mcp_config.example.json" },
                @{ Source = Join-Path $RepoRoot ".ai\adapters\codex\templates\.codex\hooks.example.md"; Dest = Join-Path $ProjectPath ".codex\hooks.example.md" }
            )
            ...
            $codexWorkflowsSourceDir = Join-Path $RepoRoot ".ai\adapters\codex\templates\.codex\workflows"
        ...
        if ($t -eq "copilot") {
            $copilotExtraFiles = @(
                @{ Source = Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.vscode\mcp.json"; Dest = Join-Path $ProjectPath ".vscode\mcp.json" },
                @{ Source = Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.vscode\tasks.hooks.json"; Dest = Join-Path $ProjectPath ".vscode\tasks.hooks.json" }
            )
            ...
            $copilotPromptsSourceDir = Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.github\prompts"
    ...
    if ($DryRun -and $t -eq "claude") {
        ...
        $claudeDryExtras = @(
            (Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\mcp_config.example.json"),
            (Join-Path $RepoRoot ".ai\adapters\claude-code\templates\.claude\hooks.example.json")
        )
    ...
    if ($DryRun -and $t -eq "codex") {
        $codexDryExtras = @(
            (Join-Path $RepoRoot ".ai\adapters\codex\templates\.codex\mcp_config.example.json"),
            (Join-Path $RepoRoot ".ai\adapters\codex\templates\.codex\hooks.example.md")
        )
    ...
    if ($DryRun -and $t -eq "copilot") {
        $copilotDryExtras = @(
            (Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.vscode\mcp.json"),
            (Join-Path $RepoRoot ".ai\adapters\vscode-copilot\templates\.vscode\tasks.hooks.json")
        )

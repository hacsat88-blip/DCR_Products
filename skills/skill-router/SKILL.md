...
1. **明示指定優先**: ユーザーが `/skill-name` を指定したらそのまま実行する
...
- オーケストレーションモード: 親カテゴリ -> 子スキルを自動選定して実行
- 個別モード: `/skill-name` を直接指定して即時実行
...
`rules/*.md` に `challenge:` frontmatter がある場合、ルーターはプライマリ実行後に自動でチャレンジャーを発火できる。

### 対抗レビューフロー

1. プライマリエージェント（例: `backend-architect`）がタスクを完了
2. ルーターが `.dcr/adversarial-pairs.yaml` を参照
3. `auto_trigger` 条件に一致するチャレンジャーを最大2件選定
